...
where
    F: Tx<Ctx, Item = T, Err = E>,
    G: Tx<Ctx, Item = F::Item, Err = F::Err>,
    H: FnOnce(F::Err) -> G,
{
    move |ctx| match f.run(ctx) {
        Ok(t) => Ok(t),
...
where
    F: Tx<Ctx, Item = T, Err = E>,
    H: FnOnce(F::Item) -> Result<U, F::Err>,
{
    move |ctx| match f.run(ctx) {
        Ok(t) => h(t),
        Err(e) => Err(e),
...
where
    F: Tx<Ctx, Item = T, Err = E1>,
    G: FnOnce(F::Err) -> Result<F::Item, E2>,
{
    move |ctx| match f.run(ctx) {
        Ok(t) => Ok(t),
        Err(e) => g(e),
...
where
    F: Tx<Ctx, Item = T, Err = E>,
    H: FnOnce(F::Item) -> Result<F::Item, F::Err>,
{
    move |ctx| match f.run(ctx) {
        Ok(t) => h(t),
        Err(e) => Err(e),
